...
## 1. Опис тестового середовища СЕВДЕІР

Тестове середовище СЕВДЕІР призначено для перевірки працездатності веб-сервісів та веб-клієнтів, які мають бути задіяні у промисловому середовищі СЕВДЕІР.

Учасники у тестовому середовищі СЕВДЕІР здійснюють електронну взаємодію «хибною» інформацією, але із застосуваннями веб-сервісів та веб-клієнтів, що мають використовуватись у промисловому середовищі СЕВДЕІР.

Обмін реальною інформацією у тестовому середовищі СЕВДЕІР заборонений.

Мережеве обладнання організації Учасника повинно бути налаштоване для тестового середовища згідно рисунку 1.

Учаснику необхідно спланувати мережеву інфраструктуру для компонентів системи.

## 2. Передумови до виконання встановлення

Для підключення до тестового середовища СЕВДЕІР установа повинна вжити наступні **попередні заходи**:

- укласти із Держателем угоду про підключення до СЕВДЕІР та подати Держателю заявку із відомостями про відповідальну особу (виконується на етапі підключення до тестового середовища);
- підготувати програмно-апаратне забезпечення під встановлення компонентів СЕВДЕІР згідно табл. 1;
- виділити 1 статичну зовнішню («білу») IP-адресу (може суміщатися з іншими тестовими сервісами організації, якщо необхідні TCP-порти вільні для використання);
- налаштувати мережевий зв'язок згідно правил, які вказано на рисунку 1;
- отримати тестову електронну печатку установи на захищеному носії особистих ключів в будь-якому сумісному акредитованому центрі сертифікації ключів (у кваліфікованого надавача електронних довірчих послуг). Див. перелік сумісних АЦСК за посиланням [https://trembita.gov.ua/ua/projects/howtoconnect](https://trembita.gov.ua/ua/projects/howtoconnect).

Для підключення до **тестового середовища** обов’язковим компонентом Учасника є шлюз безпечного обміну (компонент, позначений як «VM Шлюз безпечного обміну» на рисунку 1). Це віртуальна машина, що повинна мати вихід до мережі Інтернет, а також приймати вхідні підключення з мережі Інтернет. 

Для шлюзу безпечного обміну необхідно визначити наступні параметри:

- статична IP-адреса в локальній мережі організації;
- ім’я хоста (hostname);
- зовнішня статична IP-адреса, що відповідає за приймання вхідних підключень до шлюзу безпечного обміну, та має бути зареєстрована у СЕВДЕІР. Також, саме ця зовнішня статична IP-адреса має використовуватися шлюзом безпечного обміну для підключення до шлюзів інших організацій.
...
Тестовий шлюз безпечного обміну функціонує на базі операційної системи Ubuntu Server 16.04.5 x64. Інсталяція операційної системи виконується згідно [додатку А](member-install-ubuntu-16-04-5-lts.md) цієї інструкції.
...
- `<ADMIN_USER>` - це логін адміністратора безпеки операційної системи шлюзу безпечного обміну, який створено під час інсталяції операційної системи (згідно [додатку А](member-install-ubuntu-16-04-5-lts.md) до цієї інструкції),
