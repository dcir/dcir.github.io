...
- підготувати програмно-апаратне забезпечення під встановлення компонентів СЕВДЕІР згідно [табл. 1](#trembita-test-install-table-0-link);
...
- налаштувати мережевий зв'язок згідно правил, які вказано на [рисунку 1](#trembita-test-install-0-link);
...
Для підключення до **тестового середовища** обов’язковим компонентом Учасника є шлюз безпечного обміну (компонент, позначений як «VM Шлюз безпечного обміну» на [рисунку 1](#trembita-test-install-0-link). Це віртуальна машина, що повинна мати вихід до мережі Інтернет, а також приймати вхідні підключення з мережі Інтернет. 
...
<span id="trembita-test-install-image-0-link"></span>

![Мережа][trembita-test-install-0]

Мінімальні апаратні характеристики та використовуване програмне забезпечення (далі - ПЗ) віртуальної машини (фізичного сервера), необхідні для підключення до тестового середовища, наведені у [таблиці 1](#trembita-test-install-table-0-link).
